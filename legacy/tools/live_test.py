"""End-to-end live MT5 + Trading Stockfish validation (QUARANTINED).

This helper has been moved to legacy/ — production code must use adapters only.
"""
...
def fetch_tick(symbol: str):
    ...
    if tick is None:
        ...
        return None
    # Convert MT5 Tick (SDK object) into canonical primitive dict
    ts = None
    if hasattr(tick, "time_msc") and getattr(tick, "time_msc"):
        try:
            ts = float(getattr(tick, "time_msc")) / 1000.0
        except Exception:
            ts = None
    if ts is None and hasattr(tick, "time"):
        try:
    ...
    print("=== LIVE TICK ===")
    print(f"Symbol: {canonical['symbol']}")
    print(f"Bid: {canonical['bid']}")
    print(f"Ask: {canonical['ask']}")
    print(f"Last: {canonical['last']}")
    print(f"Time: {canonical['timestamp']}")
    return canonical
