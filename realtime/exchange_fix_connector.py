...
class FIXConnector(BaseConnector):
    ...
    def _create_message(self, msg_type: str, fields: Dict[str, str] = None) -> str:
        ...
        msg = (
            f"8={self.version.value}|9={len(msg)}|{msg}93={len(msg)}|10={checksum:03d}|"
        )
    ...
    def _create_logon_message(self) -> str:
        ...
        return self._create_message(
            "1",
            {"98": "0", "108": "30"},  # EncryptMethod: None  # HeartBtInt: 30 seconds
        )
    ...
    def unsubscribe(self, symbol: str):
        ...
        try:
            msg = self._create_message(
                "V", {"262": f"sub_{symbol}", "263": "2", "55": symbol}  # Unsubscribe
            )
            self._send_message(msg)
        ...
        try:
            ...
            msg = self._create_message(
                "D",
                {
                    ...
                    .replace(microsecond=0)
                    .isoformat()
                    .replace("+00:00", "Z"),  # TransactTime
                    "38": str(int(order.quantity)),  # OrderQty
                    "40": (
                        "1" if order.order_type == OrderType.MARKET else "2"
                    ),  # OrdType
                },
            )
            ...
            logger.info(
                f"FIX: Submitted {order.side.value} {order.quantity} {order.symbol}"
            )
